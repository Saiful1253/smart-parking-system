...
async function refreshZones() {
    ...
    try {
        ...
        var zonesLoaded = await loadZonesFromAPI();
        if (!zonesLoaded) {
            ...
            if (token && localSession) {
                try {
                    ...
                    if (sessionsResponse.ok) {
                        ...
                        const match = allSessions.find(function(bs) { return bs.plateNumber === localSession.vehicle && bs.zone === localSession.zone && (bs.status === 'Active' || bs.status === 'Parked'); });
                        if (match) {
...
function initAdminMap() {
    ...
    zones.forEach(function(z) {
        ...
        var icon = L.divIcon({
            html: '<div style="background:' + color + ';width:30px;height:30px;border-radius:50%;display:flex;align-items:center;justify-content:center;border:3px solid #152238;box-shadow:0 2px 8px rgba(0,0,0,0.3);font-size:11px;color:white;font-weight:bold;font-family:Poppins,sans-serif;">' + z.name.replace('Zone ','') + '</div>',
