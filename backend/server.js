...
// Legacy URL rewrites for moved frontend assets
app.use((req, res, next) => {
    const pathLower = req.path.toLowerCase();
    if (pathLower === '/index.html' || pathLower.match(/^\/(admin|book-parking|customer|my-sessions|my-history|admin-cash|admin-customer|admin-history|admin-map|admin-sessions|admin-settings|admin-zones)\.html$/)) {
        req.url = '/docs' + req.path;
        return next();
    }
    if (pathLower === '/style.css') {
        req.url = '/docs/css/style.css';
        return next();
    }
    ...
    if (pathLower.match(/^\/(script|admin|env|update-backend-url)\.js$/)) {
        req.url = '/docs/js' + req.path;
        return next();
    }
    if (pathLower.startsWith('/js/')) {
        req.url = '/docs' + req.path;
        return next();
    }
    next();
...
// Serve LPR temp images
app.use('/temp', express.static(path.join(__dirname, 'data', 'temp'), {
    maxAge: 0,
    etag: false,
    lastModified: false
}));
